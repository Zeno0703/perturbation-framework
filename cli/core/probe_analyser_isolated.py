...
def evaluate(probe_id, tests, project_dir, agent_jar, target_package,
    ...
    # THE EXPERIMENT: Loop through each test and give it a fresh JVM!
    for test in sorted(tests):
        # Run Maven for JUST this one test
        code, stderr, timed_out = run_maven(
            probe_id, project_dir, agent_jar, target_package,
            timeout_limit, targeted_tests=[test], maven_goal="surefire:test"
        )

        # Read the artifacts for this specific isolated run
        outcomes = read_test_outcomes(project_dir)
        actions = read_perturbations(project_dir)

        # Store the actions for this test
        for t, acts in actions.items():
            master_actions_map[t] = acts

        status = outcomes.get(test)

        # 1. Did this specific isolated test hang the JVM?
        if timed_out:
            any_timeout = True
            test_results_dict[test] = "FAIL (TIMEOUT)"
            log_file.write(f"  - {test}: FAIL (TIMEOUT) [Isolated run hung]\n")
            failed_count += 1

        # 2. Did the JVM crash instantly before writing an outcome?
        elif not outcomes and code != 0:
            test_results_dict[test] = "FAIL (JVM CRASH)"
            log_file.write(f"  - {test}: FAIL (JVM CRASH) [Violent exit before test finished]\n")
            failed_count += 1

        # 3. Did it crash mid-execution?
        elif status == "STARTED":
            test_results_dict[test] = "FAIL (JVM CRASH)"
            log_file.write(f"  - {test}: FAIL (JVM CRASH) [Violent exit during execution]\n")
            failed_count += 1

        # 4. Did it finish normally?
        elif status:
            test_results_dict[test] = status
            test_actions = actions.get(test, [])
        ...
        # 5. Failsafe
            ...
            log_file.write(f"  - {test}: UNREACHED (No outcome recorded in isolated run)\n")

    return test_results_dict, passed_count, failed_count, any_timeout, master_actions_map
